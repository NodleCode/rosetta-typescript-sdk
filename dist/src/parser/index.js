...
class RosettaParser {
    ...
    balanceChanges(block, blockRemoved) {
        ...
        for (let tx of block.transactions) {
            for (let op of tx.operations) {
                ...
                if (blockRemoved) {
                    const negatedValue = utils_1.NegateValue(amount.value);
                    amount.value = negatedValue;
                    blockIdentifier = block.parent_block_identifier;
                }
                const serializedAccount = utils_1.Hash(op.account);
                const serializedCurrency = utils_1.Hash(op.amount.currency);
    ...
    addOperationToGroup(operationsGroup = EMPTY_OPERATIONS_GROUP, destinationIndex, assignmentsArray = [], operation) {
        ...
        if (-1 == operationsGroup.currencies.findIndex(curr => utils_1.Hash(curr) == utils_1.Hash(operation.amount.currency))) {
    ...
    operationMatch(operation, operationsDescriptionArray, matchesArray) {
        for (let i = 0; i < operationsDescriptionArray.length; ++i) {
            ...
            if (operation.amount != null) {
                const val = utils_1.AmountValue(operation.amount);
    ...
    equalAmounts(operationsArray) {
        ...
        const val = utils_1.AmountValue(operationsArray[0].amount);
        for (let op of operationsArray) {
            const otherVal = utils_1.AmountValue(op.amount);
            if (val !== otherVal) {
    ...
    oppositeAmounts(operationA, operationB) {
        const valA = utils_1.AmountValue(operationA.amount);
        const valB = utils_1.AmountValue(operationB.amount);
