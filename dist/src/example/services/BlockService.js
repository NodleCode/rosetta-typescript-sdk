...
const block = async (params) => {
    ...
    if (blockRequest.block_identifier.index != 1000) {
        ...
        const blockIdentifier = new Types.BlockIdentifier(blockRequest.block_identifier.index, `block ${blockRequest.block_identifier.index}`);
        const parentBlockIdentifier = new Types.BlockIdentifier(previousBlockIndex, `block ${previousBlockIndex}`);
        const timestamp = Date.now() - 500000;
        const transactions = [];
        const block = new Types.Block(blockIdentifier, parentBlockIdentifier, timestamp, transactions);
