...
const { AccountCurrency: _AccountCurrency } = __1.Client;
...
class RosettaReconciler {
    ...
    async queueChanges(blockIdentifier, balanceChangesArray) {
        ...
        for (let change of balanceChangesArray) {
            await this.inactiveAccountQueue(false, new _AccountCurrency(change.account, change.currency), blockIdentifier);
    ...
    async compareBalance(accountIdentifier, currency, amount, blockIdentifier) {
        ...
        const difference = utils_1.SubtractValues(cachedBalance.value, amount);
        return {
            difference,
    ...
    async bestBalance(accountIdentifier, currency, partialBlockIdentifier) {
        ...
        return await this.getCurrencyBalance(this.fetcher, this.network, accountIdentifier, currency, partialBlockIdentifier);
    }
    ...
    async accountReconciliation(accountIdentifier, currency, amount, blockIdentifier, isInactive) {
        ...
        while (true) {
            ...
            let headIndex;
            try {
                const result = await this.compareBalance(accountIdentifier, currency, amount, blockIdentifier);
                ({ difference, cachedBalance, headIndex } = result);
            }
            catch (e) {
                if (e instanceof errors_1.ReconcilerError) {
                    switch (e.type) {
                        case RECONCILIATION_ERROR_HEAD_BEHIND_LIVE: {
                            ...
                            if (diff < this.waitToCheckDiff) {
                                await sleep_1.default(this.waitToCheckDiffSleep);
                                continue;
                            }
            ...
            if (difference != "0") {
                const error = await this.handler.reconciliationFailed(reconciliationType, accountCurrency.account_identifier, accountCurrency.currency, cachedBalance, amount, blockIdentifier);
                if (error)
                    throw error;
            }
            await this.inactiveAccountQueue(isInactive, accountCurrency, blockIdentifier);
            return await this.handler.reconciliationSucceeded(reconciliationType, accountCurrency.account_identifier, accountCurrency.currency, amount, blockIdentifier);
        }
    }
    static ContainsAccountCurrency(accountCurrencyMap, accountCurrency) {
        const element = accountCurrencyMap[utils_1.Hash(accountCurrency)];
        return element != null;
    }
    ...
    async reconcileActiveAccounts() {
        while (true) {
            ...
            if (balanceChange.block.index < this.highWaterMark)
                continue;
            const { block, value } = await this.bestBalance(balanceChange.account_identifier, balanceChange.currency, utils_1.constructPartialBlockIdentifier(balanceChange.block));
            await this.accountReconciliation(balanceChange.account_identifier, balanceChange.currency, value, block, false);
        }
    }
    async reconcileInactiveAccounts() {
        while (true) {
            ...
            if (!shouldAttempt) {
                await sleep_1.default(RECONCILIATION_INACTIVE_SLEEP_MS);
                continue;
            }
            ...
            if (nextValidIndex <= head.index) {
                ...
                const { block, amount } = await this.bestBalance(nextAccount.entry.account_identifier, nextAccount.entry.currency, utils_1.constructPartialBlockIdentifier(head));
                await this.accountReconciliation(nextAccount.entry.account_identifier, nextAccount.entry.currency, amount, block, true);
    ...
    async getCurrencyBalance(fetcher, networkIdentifier, accountIdentifier, currency, partialBlockIdentifier) {
        const { liveBlock, liveBalances } = await fetcher.AcountBalanceRetry(networkIdentifier, accountIdentifier, partialBlockIdentifier);
        try {
