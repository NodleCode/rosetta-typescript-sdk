...
class RosettaAsserter {
  ...
  SupportedNetworks(supportedNetworks) {
    ...
    for (let network of supportedNetworks) {
      ...
      if (parsedNetworks.includes(Hash(network))) {
        throw new AsserterError(
          `SupportedNetwork has a duplicate: ${JSON.stringify(network)}`
        );
      }
  ...
  ConstructionCombineRequest(constructionCombineRequest) {
    ...
    if (
      typeof constructionCombineRequest.unsigned_transaction !== "string" ||
  ...
  ConstructionPayloadsResponse(constructionPayloadsResponse) {
    ...
    if (
      typeof constructionPayloadsResponse.unsigned_transaction !== "string" ||
  ...
  checkHex(hexPayload) {
    ...
    return (
      hexPayload.match(/^[0-9a-fA-F]+$/) != null && hexPayload.length % 2 == 0
    );
  }
  ...
  SignatureType(signatureType) {
    switch (signatureType) {
      ...
      default:
        throw new AsserterError(
          `${JSON.stringify(signatureType)} is not a supported SignatureType`
        );
    }
  }
  ...
  containsCurrency(currencies, currency) {
    let currencyIndex = currencies.findIndex((a) => Hash(a) == Hash(currency));
