...
class Controller {
  ...
  static collectFile(request, fieldName) {
    ...
    if (request.files && request.files.length > 0) {
      ...
      if (fileObject) {
        ...
        fs.renameSync(
          path.join(defaultConfig.FILE_UPLOAD_PATH, fileObject.filename),
          path.join(defaultConfig.FILE_UPLOAD_PATH, uploadedFileName)
        );
      }
    }
  ...
  static async handleRequest(request, response) {
    try {
      ...
      Controller.sendResponse(response, wrapped, defaultConfig.BEAUTIFY_JSON);
    } catch (error) {
